...
/**
 * The abstract condition-query of BURI_ALL_ROUND_STATE.
 * @author DBFlute(AutoGenerator)
 */
public abstract class AbstractBsBuriAllRoundStateCQ extends AbstractConditionQuery {

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public AbstractBsBuriAllRoundStateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
     ...
     * {in (select INTERNAL_PK_VALUE from DOCUMENT where ...)} <br />
     * DOCUMENT by my INTERNAL_PK_VALUE, named 'document'. <br />
     * This relation is auto-detected as implicit reverse FK.
     * @param subQuery The sub-query of Document for 'in-scope'. (NotNull)
     */
    public void inScopeDocument(SubQuery<DocumentCB> subQuery) {
        assertObjectNotNull("subQuery<DocumentCB>", subQuery);
        DocumentCB cb = new DocumentCB(); cb.xsetupForInScopeRelation(this); subQuery.query(cb);
        String subQueryPropertyName = keepInternalPkValue_InScopeRelation_Document(cb.query()); // for saving query-value.
        registerInScopeRelation(cb.query(), "INTERNAL_PK_VALUE", "ID", subQueryPropertyName, "document");
    }
    public abstract String keepInternalPkValue_InScopeRelation_Document(DocumentCQ subQuery);

    /**
     * Set up NotInScopeRelation (sub-query). <br />
     * {not in (select INTERNAL_PK_VALUE from DOCUMENT where ...)} <br />
     * DOCUMENT by my INTERNAL_PK_VALUE, named 'document'. <br />
     * This relation is auto-detected as implicit reverse FK.
     * @param subQuery The sub-query of Document for 'not in-scope'. (NotNull)
     */
    public void notInScopeDocument(SubQuery<DocumentCB> subQuery) {
        assertObjectNotNull("subQuery<DocumentCB>", subQuery);
        DocumentCB cb = new DocumentCB(); cb.xsetupForInScopeRelation(this); subQuery.query(cb);
        String subQueryPropertyName = keepInternalPkValue_NotInScopeRelation_Document(cb.query()); // for saving query-value.
        registerNotInScopeRelation(cb.query(), "INTERNAL_PK_VALUE", "ID", subQueryPropertyName, "document");
    }
    public abstract String keepInternalPkValue_NotInScopeRelation_Document(DocumentCQ subQuery);
    ...
    protected String xabCB() { return BuriAllRoundStateCB.class.getName(); }
    protected String xabCQ() { return BuriAllRoundStateCQ.class.getName(); }
