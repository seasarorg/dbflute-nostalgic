...
import dbflute.ldb.allcommon.cbean.grouping.LdGroupingRowEndDeterminer;
 ...
 */
public class LdListResultBean<ENTITY> implements List<ENTITY>, Serializable {
    ...
    protected LdOrderByClause _orderByClause = new LdOrderByClause();
     ...
     */
    public <ROW> List<ROW> groupingList(LdGroupingRowSetupper<ROW, ENTITY> groupingRowSetupper, LdGroupingOption<ENTITY> groupingOption) {
        final List<ROW> groupingList = new ArrayList<ROW>();
        LdGroupingRowEndDeterminer<ENTITY> rowEndDeterminer = groupingOption.getGroupingRowEndDeterminer();
        if (rowEndDeterminer == null) {
            rowEndDeterminer = new LdGroupingRowEndDeterminer<ENTITY>() {
                public boolean determine(LdGroupingRowResource<ENTITY> rowResource, ENTITY nextEntity) {
                    return rowResource.isSizeUpBreakCount();
                }
            }; // as Default
        }
        LdGroupingRowResource<ENTITY> rowResource = new LdGroupingRowResource<ENTITY>();
        ...
        for (ENTITY entity : _selectedList) {
            ...
            if (rowEndDeterminer.determine(rowResource, nextElement)) { // Determine the row end!
                ...
                // Initialize!
                rowResource = new LdGroupingRowResource<ENTITY>();
                rowElementIndex = 0;
                ++allElementIndex;
                continue;
            }
    ...
    //                                                                             Mapping
    //                                                                             =======
    public <DTO> LdListResultBean<DTO> mappingList(LdEntityDtoMapper<ENTITY, DTO> entityDtoMapper) {
        final LdListResultBean<DTO> mappingList = new LdListResultBean<DTO>();
        for (ENTITY entity : _selectedList) {
     ...
     */
    public void setOrderByClause(LdOrderByClause orderByClause) {
