...
import dbflute.ldb.allcommon.exception.LdBatchEntityAlreadyUpdatedException;

/**
 * @author DBFlute(AutoGenerator)
 */
public abstract class LdInternalAbstractBatchAutoHandler extends LdInternalAbstractAutoHandler {

    // ===================================================================================
    //                                                                          Definition
    //                                                                          ==========
    /** Log instance. */
    private static final org.apache.commons.logging.Log _log = org.apache.commons.logging.LogFactory.getLog(LdInternalAbstractBatchAutoHandler.class);

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public LdInternalAbstractBatchAutoHandler(DataSource dataSource,
    ...
    protected void handleBatchUpdateResultWithOptimisticLockByUpdateCount(List<?> list, int updateCount) {
        ...
        if (updateCount < entityCount) {
            if (isOptimisticLockHandling()) {
                throw new LdBatchEntityAlreadyUpdatedException(list.get(0), 0, updateCount);
            } else {
    ...
    protected void handleBatchUpdateResultWithOptimisticLockByResult(List<?> list, int[] result) {
        ...
        if (alreadyUpdated) {
            ...
            if (isOptimisticLockHandling()) {
                throw new LdBatchEntityAlreadyUpdatedException(list.get(index), 0, updateCount);
            } else {
                String msg = "The entity have already deleted:";
                msg = msg + " entity=" + list.get(index);
                msg = msg + " updateCount=" + updateCount;
