...
import dbflute.ldb.allcommon.cbean.LdMapParameterBean;
import dbflute.ldb.allcommon.cbean.coption.LdLikeSearchOption;
import dbflute.ldb.allcommon.exception.LdBindVariableCommentNotFoundPropertyException;
import dbflute.ldb.allcommon.exception.LdBindVariableParameterNullValueException;
import dbflute.ldb.allcommon.exception.LdEmbeddedValueCommentNotFoundPropertyException;
import dbflute.ldb.allcommon.exception.LdEmbeddedValueParameterNullValueException;
import dbflute.ldb.allcommon.exception.LdEndCommentNotFoundException;
import dbflute.ldb.allcommon.exception.LdIfCommentConditionNotFoundException;
import dbflute.ldb.allcommon.exception.LdIfCommentNotBooleanResultException;
import dbflute.ldb.allcommon.exception.LdIfCommentWrongExpressionException;
 ...
 */
public class LdInternalSqlParser {
    ...
    protected LdInternalSqlTokenizer tokenizer;
    protected Stack<Node> nodeStack = new Stack<Node>();

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public LdInternalSqlParser(String sql, boolean blockNullParameter) {
        sql = sql.trim();
        if (sql.endsWith(";")) {
        ...
        tokenizer = new LdInternalSqlTokenizer(sql);
    }
    ...
    public Node parse() {
        ...
        while (LdInternalSqlTokenizer.EOF != tokenizer.next()) {
            parseToken();
        }
        return pop();
    }
    ...
    protected void parseToken() {
        ...
        case LdInternalSqlTokenizer.SQL:
            parseSql();
            break;
        case LdInternalSqlTokenizer.COMMENT:
            parseComment();
            break;
        case LdInternalSqlTokenizer.ELSE:
            parseElse();
            break;
        case LdInternalSqlTokenizer.BIND_VARIABLE:
            parseBindVariable();
            break;
        }
    }
    ...
    protected void parseSql() {
        ...
        if ((node instanceof InternalIfNode || node instanceof ElseNode) && node.getChildSize() == 0) {
            ...
            LdInternalSqlTokenizer st = new LdInternalSqlTokenizer(sql);
            st.skipWhitespace();
    ...
    protected void throwIfCommentConditionNotFoundException() {
        ...
        msg = msg + "* * * * * * * * * */";
        throw new LdIfCommentConditionNotFoundException(msg);
    }
    ...
    protected void parseEnd() {
        while (LdInternalSqlTokenizer.EOF != tokenizer.next()) {
            if (tokenizer.getTokenType() == LdInternalSqlTokenizer.COMMENT && isEndComment(tokenizer.getToken())) {
                pop();
                return;
            }
            parseToken();
    ...
    protected void throwEndCommentNotFoundException() {
        ...
        msg = msg + "[Specified SQL]" + getLineSeparator() + specifiedSql + getLineSeparator();
        msg = msg + "* * * * * * * * * */";
        throw new LdEndCommentNotFoundException(msg);
    }
    ...
    protected AbstractNode createBindVariableNode(String expr, String testValue) {// Extension!
        return new InternalBindVariableNode(expr, testValue, specifiedSql, blockNullParameter);
    }

    protected AbstractNode createEmbeddedValueNode(String expr, String testValue) {// Extension!
        return new InternalEmbeddedValueNode(expr, testValue, specifiedSql, blockNullParameter);
    }
    ...
    protected static class InternalValueAndTypeSetuper {
        ...
        protected void setupValueAndType(InternalValueAndType valueAndType) {
            ...
            LdLikeSearchOption likeSearchOption = null;
            ...
            for (int pos = 1; pos < _names.length; ++pos) {
                ...
                if (pos == 1 && LdMapParameterBean.class.isAssignableFrom(clazz)) {
                    final LdMapParameterBean pmb = (LdMapParameterBean)value;
        ...
        // for OutsideSql
        protected boolean isLastLoop4LikeSearch(int pos, LdLikeSearchOption likeSearchOption) {
        ...
        // for OutsideSql
        protected LdLikeSearchOption getLikeSearchOption(BeanDesc beanDesc, String currentName, Object resourceBean) {
            final PropertyDesc pb = beanDesc.getPropertyDesc(currentName + "InternalLikeSearchOption");
            final LdLikeSearchOption option = (LdLikeSearchOption) pb.getValue(resourceBean);
            if (option == null) {
        ...
        // for OutsideSql
        protected void throwOutsideSqlLikeSearchOptionSplitUnsupportedException(LdLikeSearchOption option, Object resourceBean, String currentName) {
            String msg = "Look! Read the message below." + getLineSeparator();
            msg = msg + "/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *" + getLineSeparator();
        ...
        protected String initCap(String name) {
            return LdSimpleStringUtil.initCap(name);
        }
        ...
        protected void throwBindOrEmbeddedCommentNotFoundPropertyException(String expression, Class<?> targetType, String notFoundProperty, String specifiedSql, boolean bind) {
            ...
            msg = msg + "* * * * * * * * * */";
            if (bind) {
                throw new LdBindVariableCommentNotFoundPropertyException(msg);
            } else {
                throw new LdEmbeddedValueCommentNotFoundPropertyException(msg);
            }
        }
    ...
    protected static class ParameterCommentExceptionProvider {
        public static void throwBindOrEmbeddedParameterNullValueException(String expression, Class<?> targetType, String specifiedSql, boolean bind) {
            ...
            msg = msg + "* * * * * * * * * */";
            if (bind) {
                throw new LdBindVariableParameterNullValueException(msg);
            } else {
                throw new LdEmbeddedValueParameterNullValueException(msg);
            }
        }
    ...
    protected static class InternalIfNode extends ContainerNode {
        ...
        protected void throwIfCommentWrongExpressionException(String expression, RuntimeException cause, String specifiedSql) {
            ...
            msg = msg + "[Specified SQL]" + getLineSeparator() + specifiedSql + getLineSeparator();
            msg = msg + "* * * * * * * * * */";
            throw new LdIfCommentWrongExpressionException(msg, cause);
        }
        ...
        protected void throwIfCommentNotBooleanResultException(String expression, Object result, String specifiedSql) {
            ...
            msg = msg + "[Specified SQL]" + getLineSeparator() + specifiedSql + getLineSeparator();
            msg = msg + "* * * * * * * * * */";
            throw new LdIfCommentNotBooleanResultException(msg);
        }
    ...
    public static String convertTwoWaySql2DisplaySql(String twoWaySql, String[] argNames
                                                   , Class<?>[] argTypes, Object[] args
                                                   , String logDateFormat
                                                   , String logTimestampFormat) {
        final CommandContext context;
        {
            final LdInternalSqlParser parser = new LdInternalSqlParser(twoWaySql, false);
            final Node node = parser.parse();
            final LdInternalCommandContextCreator creator = new LdInternalCommandContextCreator(argNames, argTypes);
            context = creator.createCommandContext(args);
            node.accept(context);
        }
        final String preparedSql = context.getSql();
        return LdInternalBindVariableUtil.getCompleteSql(preparedSql, context.getBindVariables(), logDateFormat, logTimestampFormat);
    }
}
