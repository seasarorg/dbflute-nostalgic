...
public class BsMemberCQ extends AbstractBsMemberCQ {
    ...
    public ConditionValue getFormalizedDatetime() {
        if (_formalizedDatetime == null) { _formalizedDatetime = new ConditionValue(); }
    ...
    public ConditionValue getBirthdate() {
        if (_birthdate == null) { _birthdate = new ConditionValue(); }
        return _birthdate;
    }
    ...
    protected MemberLoginCQ xcreateQueryMemberLoginAsLatest() {
        ...
        String jan = resolveJoinAliasName(nrp, getNextNestLevel());
        MemberLoginCQ cq = new MemberLoginCQ(this, getSqlClause(), jan, getNextNestLevel());
    ...
    protected void xsetupOuterJoinMemberLoginAsLatest() {
        ...
        joinOnMap.put("$$fixedCondition$$", ppFxCd("$$foreignAlias$$.LOGIN_DATETIME = (select max(LOGIN_DATETIME) from MEMBER_LOGIN where MEMBER_ID = $$localAlias$$.MEMBER_ID)", getRealAliasName(), cq.getRealAliasName()));
        registerOuterJoin(cq, joinOnMap);
    }
